...
//    NSCameraUsageDescription → "Pour scanner vos documents"
//    NSPhotoLibraryUsageDescription → "Pour importer des photos"
//
//  ⚠️  ML Kit ne fonctionne PAS sur web — uniquement Android/iOS
//      Sur web, le bouton scanner est désactivé avec un message explicatif
// ─────────────────────────────────────────────────────────────────────────────
...
class _OcrScanPageState extends State<OcrScanPage>
  ...
  Future<void> _scan() async {
    ...
    try {
      // ML Kit lit directement le fichier image
      ...
      // Construire le tableau à partir des positions X/Y des blocs
  ...
  // ── Parser les blocs ML Kit en tableau par position ──────────────────────────
  (List<String>, List<List<String>>) _buildTableFromBlocks(
      List<TextBlock> blocks) {

    // Collecter tous les éléments (lignes) avec leurs bounding boxes
    ...
    // 1. Grouper par lignes (cluster Y avec tolérance = hauteur moyenne / 2)
    elements.sort((a, b) => a.y.compareTo(b.y));
    final avgH = elements.map((e) => e.height).reduce((a, b) => a + b) / elements.length;
    final tolerance = (avgH * 0.6).toInt();

    final lineGroups = <List<_TextElement>>[];
    var current = [elements[0]];
    for (int i = 1; i < elements.length; i++) {
      if ((elements[i].y - current.last.y).abs() <= tolerance) {
        current.add(elements[i]);
      } else {
        lineGroups.add(List.from(current));
        current = [elements[i]];
      }
    }
    lineGroups.add(current);

    // 2. Détecter les colonnes par clustering X
    final allX = elements.map((e) => e.x + e.width / 2).toList()..sort();
    final colCenters = _clusterValues(allX, threshold: 30);

    // 3. Assigner chaque élément à sa colonne
    ...
    final grid = lineGroups.map((line) {
      final row = List.filled(colCenters.length, '');
      for (final el in line) {
        final cx = el.x + el.width / 2;
        final ci = nearestCol(cx);
    ...
    final headers = grid[0].map((h) => h.trim()).toList();
    final rows = grid.skip(1)
  ...
  // Regrouper des valeurs numériques proches en clusters
  ...
  // ── Générer Excel et naviguer vers Mapping ───────────────────────────────────
  Future<void> _generateAndMap() async {
  ...
  Future<void> _downloadExcelOnly() async {
    if (_headers.isEmpty) return;
    setState(() => _generatingExcel = true);
    try {
      final result = await _api.tableToExcel(
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Scanner un document', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: txt, letterSpacing: -0.3)),
          Text('Photo → ML Kit → Excel → Mapping', style: TextStyle(fontSize: 12, color: sub)),
        ])),
      ]),
    );
  }

  Widget _webBanner(bool dk, Color card, Color bord, Color txt, Color sub) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
          color: _amber.withOpacity(dk ? 0.1 : 0.06),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: _amber.withOpacity(0.3))),
      child: Row(children: [
        Icon(Icons.phone_android_rounded, color: _amber, size: 22),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Fonctionnalité mobile uniquement',
              style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: txt)),
          const SizedBox(height: 4),
          Text('La reconnaissance de texte ML Kit fonctionne uniquement sur Android/iOS. '
              'Lancez l\'app sur votre téléphone pour utiliser cette fonctionnalité.',
  ...
  Widget _infoCard(bool dk, Color card, Color bord, Color txt, Color sub) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
          color: _green.withOpacity(dk ? 0.08 : 0.05),
          borderRadius: BorderRadius.circular(14),
  ...
  Widget _photoSection(bool dk, Color card, Color bord, Color txt, Color sub) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Source de l\'image', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: txt)),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: _srcBtn(icon: Icons.camera_alt_rounded, label: 'Prendre une photo',
  ...
  Widget _srcBtn({required IconData icon, required String label, required String sublabel,
    ...
    bool disabled = false}) {
    return Opacity(opacity: disabled ? 0.35 : 1.0,
      child: Container(
  ...
  Widget _scanningCard(bool dk, Color card, Color bord, Color txt, Color sub) {
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 12),
        LinearProgressIndicator(backgroundColor: _violet.withOpacity(0.1),
      ...
      Container(
        ...
        child: ClipRRect(borderRadius: BorderRadius.circular(13),
          child: SingleChildScrollView(scrollDirection: Axis.horizontal,
            child: DataTable(
              headingRowColor: WidgetStateProperty.all(dk ? const Color(0xFF1e2028) : const Color(0xFFF3F4F6)),
  ...
  Widget _actionBtns() {
    return Column(children: [
      SizedBox(width: double.infinity, height: 50,
        child: DecoratedBox(
          decoration: BoxDecoration(
              gradient: LinearGradient(colors: [_green, const Color(0xFF10b981)],
                  begin: Alignment.centerLeft, end: Alignment.centerRight),
              borderRadius: BorderRadius.circular(14),
      ...
      SizedBox(width: double.infinity, height: 44,
        child: OutlinedButton.icon(
          onPressed: _generatingExcel ? null : _downloadExcelOnly,
          icon: const Icon(Icons.download_rounded, size: 16),
          label: const Text('Télécharger Excel uniquement', style: TextStyle(fontSize: 13)),
  ...
  Widget _emptyCard(bool dk, Color card, Color bord, Color txt, Color sub) {
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 12),
        Text('Aucun tableau détecté', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: txt)),
        const SizedBox(height: 8),
        Text('Conseils pour une meilleure photo :\n'
            ...
            style: TextStyle(fontSize: 12, color: sub, height: 1.6), textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  Widget _backBtn(BuildContext ctx, bool dk, Color sub) {
    return Container(width: 38, height: 38,
        decoration: BoxDecoration(color: dk ? const Color(0xFF1a1d24) : const Color(0xFFF3F4F6),
            borderRadius: BorderRadius.circular(10)),
